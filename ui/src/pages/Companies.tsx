...
export function Companies() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("companies.list.title") }]);
  }, [setBreadcrumbs, t]);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-end">
        <Button size="sm" onClick={() => openOnboarding()}>
          ...
          {t("companies.list.newCompany")}
        </Button>
      </div>

      <div className="h-6">
        {loading && <p className="text-sm text-muted-foreground">{t("companies.list.loading")}</p>}
            ...
            >
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      >
                        ...
                        {t("companies.list.deleteCompany")}
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
              ...
              <div className="flex items-center gap-3 sm:gap-5 mt-4 text-sm text-muted-foreground flex-wrap">
                <div className="flex items-center gap-1.5">
                  ...
                  <span>
                    {t("companies.list.agent", { count: agentCount })}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
                  <CircleDot className="h-3.5 w-3.5" />
                  <span>
                    {t("companies.list.issue", { count: issueCount })}
                  </span>
                </div>
                <div className="flex items-center gap-1.5 tabular-nums">
                  ...
                  <span>
                    ...
                    {company.budgetMonthlyCents > 0
                      ...
                      : <span className="text-xs ml-1">{t("companies.list.unlimitedBudget")}</span>}
                  </span>
                </div>
                <div className="flex items-center gap-1.5 ml-auto">
                  <Calendar className="h-3.5 w-3.5" />
                  <span>{t("companies.list.created")} {relativeTime(company.createdAt)}</span>
                </div>
              </div>
              ...
              {isConfirmingDelete && (
                ...
                >
                  <p className="text-sm text-destructive font-medium">
                    {t("companies.list.deleteConfirm")}
                  </p>
                  <div className="flex items-center gap-2 ml-4 shrink-0">
                    ...
                    <Button
                      ...
                      disabled={deleteMutation.isPending}
                    >
                      {deleteMutation.isPending ? t("companies.list.deleting") : t("companies.list.delete")}
                    </Button>
                  </div>
                </div>
              )}
