...
export function InstanceSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.updateError"));
    },
  });
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {t("instanceSettings.schedulerDescription")}
        </p>
      </div>
      ...
      <div className="flex gap-4 text-sm text-muted-foreground">
        ...
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {t("instanceSettings.company", { count: grouped.length })}</span>
      </div>
